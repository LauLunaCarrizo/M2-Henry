...
myProyect.innerHTML += "LautaroLuna"
...
function buildToDo(todo, index) {
  // Tu código acá:
  let toDoShell = document.createElement("div")  // crea variable que contiene un div creado en document
  toDoShell.className = "toDoShell"              // le asignamos todoShell a la clase con el mismo nombre
  var toDoText = document.createElement("span")  // creamos variable que contiene una etiqueta span en document
  toDoText.innerHTML = todo.description;         // ponemos toDoText el valor de description
  toDoText.setAttribute("id",index)              // seteamos atributo a todoText un id ingresado por parametro index
  toDoText.addEventListener("click",completeToDo)
  if(todo.complete===true){                      // si esta completado creamos clase completeText a todoText
    toDoText.className = "completeText"
  }
  toDoShell.appendChild(toDoText)                // agregamos como hijo todoText a todoShell
  return toDoShell
}
